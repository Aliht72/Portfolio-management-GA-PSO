...
plotbars<- function(data1 , data2, data3, data4,
                    cap1 = "GA1", cap2 = "GA2", cap3 = "GA3" , cap4 = "GA4" ){
  data = data1
  hues = c("black","blue","green" , "red")
  ...
  min4 = findminmax(data4)   #min(data3) - data3 [which(data3 == min(data3))+nrow(data3)]
  ...
  max4 = findminmax(data4, FALSE)   #max(data3) + data3 [which(data3 == max(data3))+nrow(data3)]
  ...
  data = data4
  df <- data.frame(x=data[,1], y=data[,2], dy = data[,3])  #dy = length of error bar  
  lines(df$x, df$y, col = hues[4])
  segments(df$x, df$y - df$dy, df$x, df$y + df$dy, col = hues[4]); 
  
  
  legend("bottomright", legend = c(cap1, cap2, cap3 , cap4), col = hues, lwd = 1,
         cex = 0.5)
}

plotbars(p1 , p2 , p3 , PSOParse,  "Arithmetic" , "Blend" , "Single" , "PSO")
